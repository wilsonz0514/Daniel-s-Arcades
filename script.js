...
const defaultSave={points:0,skips:0,passes:{mergeShield:0,hunterLens:0,patternPrism:0}};
let save=loadSave(),score=0,currentGame=null,toastTimer,mergeShieldReady=false,hunterLensActive=false,patternPrismActive=false,sessionPoints=0,gameWon=false;
if(!save.testPackGranted){
  save.skips+=1;
  Object.keys(save.passes).forEach(id=>save.passes[id]++);
  save.testPackGranted=true;
  localStorage.setItem(SAVE_KEY,JSON.stringify(save));
}
if(!save.fivePackGranted){
  save.skips=Math.max(save.skips,5);
  Object.keys(save.passes).forEach(id=>save.passes[id]=Math.max(save.passes[id],5));
  save.fivePackGranted=true;
  localStorage.setItem(SAVE_KEY,JSON.stringify(save));
}

function loadSave(){try{const raw=JSON.parse(localStorage.getItem(SAVE_KEY))||{};const passes={...defaultSave.passes,...(raw.passes||{})};if(!raw.passes&&raw.owned){Object.keys(passes).forEach(id=>passes[id]=raw.owned[id]?(raw.fivePackGranted?5:1):0)}return{...defaultSave,...raw,passes}}catch{return{...defaultSave,passes:{...defaultSave.passes}}}}
function persist(){localStorage.setItem(SAVE_KEY,JSON.stringify(save));updateWallet()}
function updateWallet(){document.querySelector('#pointBalance').textContent=save.points;document.querySelector('#shopBalance').textContent=save.points;document.querySelector('#skipCount').textContent=save.skips;document.querySelector('#accessoryCount').textContent=Object.values(save.passes).reduce((total,count)=>total+count,0)}
...
const products=[
  ...
  {id:'mergeShield',icon:'◇',type:'ONE-USE MERGE PASS',name:'TILE SHIELD',description:'Blocks the first highest-tile penalty in one Merge Maestro run.',price:250},
  {id:'hunterLens',icon:'◎',type:'ONE-USE HUNTER PASS',name:'NUMBER LENS',description:'Automatically selects one correct multiple throughout one Multiple Hunter run.',price:300},
  {id:'patternPrism',icon:'△',type:'ONE-USE PATTERN PASS',name:'PATTERN PRISM',description:'Removes one wrong choice throughout one Pattern Pulse run.',price:350}
];
...
function renderShop(){
  updateWallet();
  document.querySelector('#shopGrid').innerHTML=products.map(p=>{const count=p.id==='skip'?save.skips:save.passes[p.id];const canBuy=save.points>=p.price;return `<article class="shop-card"><div class="item-icon">${p.icon}</div><div class="item-copy"><span class="item-type">${p.type}</span><h3>${p.name}</h3><p>${p.description}</p><b class="owned-stamp">${count} USE${count===1?'':'S'} READY</b><br><button class="buy-btn" data-buy="${p.id}" ${canBuy?'':'disabled'}>★ ${p.price} · BUY ONE</button></div></article>`}).join('');
  document.querySelectorAll('[data-buy]').forEach(btn=>btn.onclick=()=>buyItem(btn.dataset.buy));
}
function buyItem(id){const p=products.find(x=>x.id===id);if(!p||save.points<p.price)return;save.points-=p.price;if(id==='skip')save.skips++;else save.passes[id]++;persist();renderShop();toast(`${p.name} · ONE USE ADDED`)}
function consumePass(id){if(!save.passes[id])return false;save.passes[id]--;persist();return true}
...
function startMerge(){mergeBoard=Array(16).fill(0);selected=[];mergeTyped='';mergeShieldReady=consumePass('mergeShield');[0,1,2,4,5,6,8,9,10,12,13,15].forEach((p,i)=>mergeBoard[p]=[3,3,5,5,7,7,9,9,3,3,5,5][i]);renderMerge()}
...
function startPattern(){patternLevel=1;patternPrismActive=consumePass('patternPrism');newPattern()}
